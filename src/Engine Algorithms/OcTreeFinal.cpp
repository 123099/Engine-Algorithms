#include "OcTreeFinal.hpp"
#include "mge\config.hpp"
#include <sstream>

Mesh* OcTreeFinal::m_debugMesh = nullptr;
ColorMaterial* OcTreeFinal::m_debugMaterial = nullptr;

OcTreeFinal::OcTreeFinal(unsigned layer, const glm::vec3 & center, const glm::vec3 & halfExtents) :
	m_layer(layer),
...
void OcTreeFinal::Insert(GameObject * gameObject)
{
...
void OcTreeFinal::RetrieveObjectsInSpaceOf(std::vector<GameObject*>& returnList, GameObject * gameObject)
{
...
void OcTreeFinal::Draw(const glm::mat4& viewMat, const glm::mat4& projectionMat)
{
	if (IsSplit() == true)
	{
		for (const auto& childNode : m_nodes)
...
std::string OcTreeFinal::ToString()
{
...
void OcTreeFinal::Split()
{
	...
	m_nodes[0] = std::make_unique<OcTreeFinal>(m_layer + 1, m_bounds.GetCenter() + glm::vec3(quarterExtents.x, quarterExtents.y, -quarterExtents.z), quarterExtents);
	m_nodes[1] = std::make_unique<OcTreeFinal>(m_layer + 1, m_bounds.GetCenter() + glm::vec3(-quarterExtents.x, quarterExtents.y, -quarterExtents.z), quarterExtents);
	m_nodes[2] = std::make_unique<OcTreeFinal>(m_layer + 1, m_bounds.GetCenter() + glm::vec3(-quarterExtents.x, quarterExtents.y, quarterExtents.z), quarterExtents);
	m_nodes[3] = std::make_unique<OcTreeFinal>(m_layer + 1, m_bounds.GetCenter() + glm::vec3(quarterExtents.x, quarterExtents.y, quarterExtents.z), quarterExtents);

	m_nodes[4] = std::make_unique<OcTreeFinal>(m_layer + 1, m_bounds.GetCenter() + glm::vec3(quarterExtents.x, -quarterExtents.y, -quarterExtents.z), quarterExtents);
	m_nodes[5] = std::make_unique<OcTreeFinal>(m_layer + 1, m_bounds.GetCenter() + glm::vec3(-quarterExtents.x, -quarterExtents.y, -quarterExtents.z), quarterExtents);
	m_nodes[6] = std::make_unique<OcTreeFinal>(m_layer + 1, m_bounds.GetCenter() + glm::vec3(-quarterExtents.x, -quarterExtents.y, quarterExtents.z), quarterExtents);
	m_nodes[7] = std::make_unique<OcTreeFinal>(m_layer + 1, m_bounds.GetCenter() + glm::vec3(quarterExtents.x, -quarterExtents.y, quarterExtents.z), quarterExtents);

	m_isSplit = true;
}

bool OcTreeFinal::IsSplit()
{
	return m_isSplit;
}